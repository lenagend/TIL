...
* 의존성 교체 어려움:
    - `CustomerService`는 `JpaCustomerRepository`에 직접 의존하고 있습니다. 따라서 테스트할 때 목(mock) 객체나 다른 구현체로 교체하기가 매우 어렵다.
    - 예를 들어, 테스트에서 데이터베이스에 접근하지 않도록 하기 위해 `JpaCustomerRepository` 대신 목 객체를 사용하려고 해도 불가능하다.
* 단위 테스트 불가능:
    - `CustomerService`의 단위 테스트를 작성하려면 `JpaCustomerRepository`도 함께 인스턴스화해야 한다. 이는 테스트가 단위 테스트가 아니라 통합 테스트로 변질되게 만든다.
    - 데이터베이스와의 연결 설정이나 실제 데이터베이스 조작이 필요하게 되어 테스트가 복잡해지고, 테스트 실행 속도도 느려진다.
* 테스트 신뢰성 저하:
    - 실제 데이터베이스를 사용하게 되면 테스트 데이터의 상태가 외부 요인에 따라 달라질 수 있다. 이는 테스트 결과의 일관성을 떨어뜨린다.
    - 외부 시스템에 의존하는 테스트는 재현성이 떨어지며, 테스트 실패 시 원인 분석이 어렵다.
